...
# PyLaunchKit - Linux Python project launcher.
#
# Make this file executable before using ./run.sh directly:
#   chmod +x run.sh
...
find_system_python() {
    local candidate=""
    local saw_python=0
    local saw_python3_without_venv=0
    ...
    for candidate in python3 python; do
        if command -v "$candidate" >/dev/null 2>&1; then
            ...
            if "$candidate" -c 'import sys; sys.exit(0 if sys.version_info[0] == 3 else 1)' >/dev/null 2>&1; then
                saw_python3_without_venv=1
            fi
        fi
    done

    if [ "$saw_python" != "1" ]; then
        echo "Python was not found."
        echo "Install Python 3 and make sure python3 or python is available from PATH."
    elif [ "$saw_python3_without_venv" = "1" ]; then
        echo "Python 3 was found, but the venv module is not available."
        echo "On Debian or Ubuntu, install python3-venv and try again."
    else
        echo "Python was found, but no usable Python 3 interpreter is available."
        echo "Install Python 3 and make sure python3 or python points to it."
    fi
...
create_venv() {
    ...
    if [ "$?" -ne 0 ]; then
        ...
        cat "$LOG_FILE"
        echo
        echo "If this is a Debian or Ubuntu system, install python3-venv and try again."
        return 1
    fi
...
maybe_install_requirements() {
    ...
    current_hash="$(calculate_file_hash "$REQ_FILE")" || {
        ...
        echo "Install sha256sum or openssl and try again."
        return 1
    }
...
REQ_HASH_FILE="$VENV_DIR/.req.txt.sha256"
